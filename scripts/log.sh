...
# @example ./scripts/log.sh error "Something happened!"
error() {
...
# @example ./scripts/log.sh info "Here is some information"
info() {
    if [ -n "$ENHANCED_LOGGING" ]; then
        logger info "$1"
    else
...
# @example ./scripts/log.sh md "[styled_link](https://google.com)"
# @example ./scripts/log.sh md mymarkdown/file.md
md() {
    if [ -n "$ENHANCED_LOGGING" ]; then
        logger info "$1"
    fi
...
# @example ./scripts/log.sh star "Congratulations"
star() {
    if [ -n "$ENHANCED_LOGGING" ]; then
        logger star "$1"
    else
...
# @example ./scripts/log.sh success "Yay!"
success() {
    if [ -n "$ENHANCED_LOGGING" ]; then
        logger success "$1"
    else
...
# @example ./scripts/log.sh warn "Just so you know.."
warn() {
    if [ -n "$ENHANCED_LOGGING" ]; then
        logger warn "$1"
    else
